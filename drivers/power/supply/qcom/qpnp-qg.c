...
static int oem_cycle_count_init(struct qpnp_qg *chip);
static void oem_cycle_dnand_work(struct work_struct *work);
static void oem_online_dnand_work(struct work_struct *work);
static void oem_online_time_work(struct work_struct *work);
static int oem_cont_chg_save(struct qpnp_qg *chip);
static int oem_cont_chg_check(struct qpnp_qg *chip);
static int oem_cont_chg_init(struct qpnp_qg *chip);
static void oem_save_batt_care_status(struct qpnp_qg *chip);
static void oem_batt_care_work(struct work_struct *work);
static int oem_get_batt_care_capacity(struct qpnp_qg *chip, int *val);
static int ome_find_batt_care_votables(struct qpnp_qg *chip);
static int oem_suspend_votable(struct qpnp_qg *chip, bool suspend_enable);
static int oem_control_batt_care_param(struct qpnp_qg *chip);
static void oem_ui_soc_check_work(struct work_struct *work);
static int oem_get_current_time(unsigned long *now_tm_sec);
static void oem_batt_care_notification_check(struct qpnp_qg *chip);
#ifdef CONFIG_OEM_BMS
...
static void process_udata_work(struct work_struct *work)
{
	...
	if (chip->oem_mode_change_flag && chip->oem_real_soc <= DEFAULT_BATT_CARE_MODE_CAPACITY) {
		rc = oem_control_batt_care_param(chip);
		rc = oem_suspend_votable(chip, false);
	}
			       ...
			       const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_BATTERY_CARE_MODE:
		if (chip->oem_batt_care_mode != pval->intval) {
			chip->oem_mode_change_flag = true;
			pr_info("ui_soc correction at battery care mode change is start.\n");
			chip->oem_batt_care_mode = pval->intval;
			oem_save_batt_care_status(chip);

			rc = ome_find_batt_care_votables(chip);
			if (rc == 0) {
				rc = oem_suspend_votable(chip, true);

				if (chip->oem_batt_care_mode == BATT_CARE_MODE_OFF || chip->oem_real_soc <= DEFAULT_BATT_CARE_MODE_CAPACITY) {
					rc = oem_control_batt_care_param(chip);
					rc = oem_suspend_votable(chip, false);
				}
			}

			if (chip->oem_batt_care_mode == BATT_CARE_MODE_OFF) {
				chip->oem_batt_care_boot_flag = false;
				if (!chip->oem_ui_soc_increasing_flag) {
					chip->oem_ui_soc_increasing_flag = true;
				}
				schedule_delayed_work(&chip->oem_ui_soc_check_work, msecs_to_jiffies(BATT_CARE_SOC_CHECK_CYCLE));
			} else {
				cancel_delayed_work_sync(&chip->oem_ui_soc_check_work);
			}
	...
	case POWER_SUPPLY_PROP_BATTERY_CARE_NOTIFICATION:
		...
		if (chip->oem_batt_care_notification == 0) {
			oem_get_current_time(&chip->start_time);
			chip->connect_time = 0;
			chip->total_connect_time = 0;
			chip->lapsed_time = 0;
			chip->oem_notification_flag = false;
			chip->oem_capacity_flag = true;
		}
		break;
	default:
		break;
			       ...
			       union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		chip->oem_real_soc = pval->intval;
		if (!chip->oem_ui_soc) {
			chip->oem_ui_soc = chip->oem_real_soc;
		}

		if (chip->oem_batt_care_mode == BATT_CARE_MODE_ON) {
			if (chip->oem_mode_change_flag) {
				if (get_client_vote(chip->usb_icl_votable, BATT_CARE_VOTER) == 0) {
					pr_debug("When ui_soc correction is in progress and DCDC is OFF.");
					if (!chip->oem_ui_soc_increasing_flag) {
						chip->oem_ui_soc = chip->oem_real_soc;
					}
				} else {
					pr_debug("When ui_soc correction is in progress and DCDC is ON.");
					chip->oem_ui_soc = min(chip->oem_real_soc, DEFAULT_BATT_CARE_MODE_CAPACITY);
				}
				pval->intval = chip->oem_ui_soc;
			} else {
				rc = oem_get_batt_care_capacity(chip, &pval->intval);
			}
		} else {
			if (chip->oem_mode_change_flag) {
				pval->intval = chip->oem_ui_soc;
		...
		if (chip->oem_batt_care_boot_flag) {
			pval->intval = min(pval->intval, DEFAULT_BATT_CARE_MODE_CAPACITY);
		}
		break;
		...
		if (chip->oem_mode_change_flag) {
			if (chip->oem_batt_care_mode == BATT_CARE_MODE_ON) {
				if (chip->charge_status == POWER_SUPPLY_STATUS_FULL &&
						chip->charge_full) {
					chip->oem_mode_change_flag = false;
					pr_info("ui_soc correction at battery care mode from OFF to ON is ended.\n");
				}
			} else {
				if (chip->oem_ui_soc >= chip->oem_real_soc) {
					chip->oem_mode_change_flag = false;
					pr_info("ui_soc correction at battery care mode from ON to OFF is ended.\n");
					chip->oem_ui_soc_increasing_flag = false;
					cancel_delayed_work_sync(&chip->oem_ui_soc_check_work);
				}
			}
		}

		oem_batt_care_notification_check(chip);
	}
out:
	pm_relax(chip->dev);
}
...
static int qg_load_battery_profile(struct qpnp_qg *chip)
{
	...
	if (of_find_property(profile_node, "oem,cont-chg-factor-care-mode", &byte_len) &&
			chip->bp.oem_cont_chg_factor_levels > 0) {
		chip->bp.oem_cont_chg_factor_care_mode = devm_kzalloc(chip->dev, byte_len,
			GFP_KERNEL);

		if (chip->bp.oem_cont_chg_factor_care_mode == NULL) {
			pr_err("Couldn't read cont_chg_factor rc = %d\n", rc);
			chip->bp.oem_cont_chg_factor_levels = -EINVAL;
		} else {
			chip->bp.oem_cont_chg_factor_levels = byte_len / sizeof(u32);
			rc = of_property_read_u32_array(profile_node,
					"oem,cont-chg-factor-care-mode",
					chip->bp.oem_cont_chg_factor_care_mode,
					chip->bp.oem_cont_chg_factor_levels);
			if (rc < 0) {
				pr_err("Couldn't cont_chg_factor limits rc = %d\n", rc);
				chip->bp.oem_cont_chg_factor_levels = -EINVAL;
	...
	if (rc < 0) {
		pr_err("Failed to read QG profile version rc:%d\n", rc);
		chip->oem_batt_care_float_volt_uv = -EINVAL;
	}
...
static int oem_cycle_count_save(struct qpnp_qg *chip)
{
	int rc = 0;
	u16 cyc_count = chip->oem_cycle.count;
	u8 chg_inc = chip->oem_cycle.increase_soc;

	mutex_lock(&chip->oem_cycle.lock);

	rc = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_CYCLE_COUNT,
			(u8 *)&cyc_count, sizeof(cyc_count));
	if (rc < 0) {
		pr_err("Failed to dnand write:DNAND_OFFSET_CYCLE_COUNT\n");
	}

	rc = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_CYC_CTR_CHG_INC,
			(u8 *)&chg_inc, sizeof(chg_inc));
	if (rc < 0) {
		pr_err("Failed to dnand write:DNAND_OFFSET_CYC_CTR_CHG_INC\n");
	}

	mutex_unlock(&chip->oem_cycle.lock);
...
static int oem_cycle_count_check(struct qpnp_qg *chip)
{
	...
	if (!chip->oem_cycle.dnand_loaded) {
		rc = oem_cycle_count_init(chip);
	}
	if (rc < 0) {
		pr_err("Failed to dnand read, rc:%d\n", rc);
		goto out;
	}
...
static int oem_cycle_count_init(struct qpnp_qg *chip)
{
	...
	rc = kdnand_id_read(DNAND_ID_KERNEL_23, DNAND_OFFSET_CYCLE_COUNT,
		(uint8_t*)&cyc_count, sizeof(cyc_count));
	if (rc == DNAND_NO_ERROR) {
		chip->oem_cycle.dnand_loaded = true;
		chip->oem_cycle.count = cyc_count;
	} else {
		chip->oem_cycle.dnand_loaded = false;
	}

	if (chip->oem_cycle.dnand_loaded) {
		rc = kdnand_id_read(DNAND_ID_KERNEL_23, DNAND_OFFSET_CYC_CTR_CHG_INC,
			(uint8_t*)&chg_inc, sizeof(chg_inc));
		if (rc == DNAND_NO_ERROR) {
			chip->oem_cycle.dnand_loaded = true;
			chip->oem_cycle.increase_soc = chg_inc;
		} else {
			chip->oem_cycle.dnand_loaded = false;
		}
	}
...
static void oem_cycle_dnand_work(struct work_struct *work)
{
	struct qpnp_qg *chip = container_of(work, struct qpnp_qg,
					    oem_cycle_dnand_work.work);
	int dnand_loaded = 0;
	int rc = 0;
	static int read_err_count = 0;

	dnand_loaded = oem_cycle_count_init(chip);

	if (dnand_loaded < 0) {
		read_err_count++;
		pr_err("Failed to dnand read, rc:%d, read_err_count:%d\n", dnand_loaded, read_err_count);
		if (read_err_count > DNAND_READ_ERR_COUNT) {
			pr_info("DNAND clear, read_err_count:%d\n", read_err_count);
			chip->oem_cycle.count = 0;
			chip->oem_cycle.increase_soc = 0;
			rc = oem_cycle_count_save(chip);
		}
		schedule_delayed_work(&chip->oem_cycle_dnand_work, msecs_to_jiffies(DNAND_CHECK_CYCLE));
	} else {
		oem_thresh_check(chip, CYCLE_COUNT);
...
static void oem_online_time_work(struct work_struct *work)
{
	...
	if (chip->oem_batt_care_mode == BATT_CARE_MODE_ON) {
		check_capacity = TIME_CHECK_CAPACITY_CARE_MODE;
	}
	...
	for (i = (chip->bp.oem_cont_chg_factor_levels - 1); i >= 0; i--) {
		if (batt_temp >= chip->bp.oem_batt_temp_thresh[i]) {
			if (chip->oem_batt_care_mode == BATT_CARE_MODE_ON) {
				factor_cont_chg = chip->bp.oem_cont_chg_factor_care_mode[i];
			} else {
...
{
	int rc = 0;
	u32 total_connect_time = chip->oem_online.total_time;

	mutex_lock(&chip->oem_online.lock);

	rc = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_TOTAL_CONNECT_TIME,
			(u8 *)&total_connect_time, sizeof(total_connect_time));
	if (rc < 0) {
		pr_err("Failed to dnand write:DNAND_OFFSET_TOTAL_CONNECT_TIME\n");
	}

	mutex_unlock(&chip->oem_online.lock);
...
static int oem_cont_chg_check(struct qpnp_qg *chip)
{
	...
	if (!chip->oem_online.dnand_loaded) {
		rc = oem_cont_chg_init(chip);
	}
	if (rc < 0) {
		pr_err("Failed to dnand read, rc:%d\n", rc);
		goto out;
	...
	if (chip->charge_status == POWER_SUPPLY_STATUS_CHARGING) {
		...
		if (last_total_time != chip->oem_online.total_time) {
			rc = oem_cont_chg_save(chip);
		}
...
{
	int rc = 0;
	u32 total_connect_time;

	mutex_lock(&chip->oem_online.lock);

	rc = kdnand_id_read(DNAND_ID_KERNEL_23, DNAND_OFFSET_TOTAL_CONNECT_TIME,
		(uint8_t*)&total_connect_time, sizeof(total_connect_time));
	if (rc == DNAND_NO_ERROR) {
		chip->oem_online.dnand_loaded = true;
		chip->oem_online.total_time = total_connect_time;
	} else {
		chip->oem_online.dnand_loaded = false;
	}
...
{
	struct qpnp_qg *chip = container_of(work, struct qpnp_qg,
					    oem_online_dnand_work.work);
	int dnand_loaded = 0;
	int rc = 0;
	static int read_err_count = 0;

	dnand_loaded = oem_cont_chg_init(chip);
	...
	if (dnand_loaded < 0) {
		...
		if (read_err_count > DNAND_READ_ERR_COUNT) {
			pr_info("DNAND clear, read_err_count:%d\n", read_err_count);
			chip->oem_online.total_time = 0;
			rc = oem_cont_chg_save(chip);
		}
		schedule_delayed_work(&chip->oem_online_dnand_work, msecs_to_jiffies(DNAND_CHECK_CYCLE));
	} else {
		oem_thresh_check(chip, CONT_CHG);
...
static void oem_save_batt_care_status(struct qpnp_qg *chip)
{
	...
	if (chip->oem_read_complete_flag) {
		mutex_lock(&chip->oem_batt_care_lock);

			rc = kdnand_id_write(DNAND_ID_KERNEL_23, DNAND_OFFSET_BATT_CARE_STATUS,
					(u8 *)&chip->oem_batt_care_mode, sizeof((u8)chip->oem_batt_care_mode));

		mutex_unlock(&chip->oem_batt_care_lock);
	}
}

static void oem_restore_batt_care_status(struct qpnp_qg *chip)
{
	int rc;
	u8 batt_care_status = 0;
	union power_supply_propval prop = {0, };

	if (!chip->oem_read_complete_flag) {
		mutex_lock(&chip->oem_batt_care_lock);

		rc = kdnand_id_read(DNAND_ID_KERNEL_23, DNAND_OFFSET_BATT_CARE_STATUS,
				(uint8_t*)&batt_care_status, sizeof(batt_care_status));
		if (rc == DNAND_NO_ERROR) {
			chip->oem_read_complete_flag = true;
		}

		mutex_unlock(&chip->oem_batt_care_lock);

		if (chip->oem_read_complete_flag) {
			if (chip->oem_batt_care_mode == BATT_CARE_MODE_INIT) {
				prop.intval = (int)batt_care_status;
			...
			if ((int)batt_care_status == BATT_CARE_MODE_ON) {
				chip->oem_batt_care_boot_flag = true;
				pr_info("Power on when battery care mode is on\n");
			}
		}
	}
}
...
{
	struct qpnp_qg *chip = container_of(work, struct qpnp_qg,
					    oem_batt_care_work.work);

	oem_restore_batt_care_status(chip);

	if (!chip->oem_read_complete_flag) {
		schedule_delayed_work(&chip->oem_batt_care_work, msecs_to_jiffies(BATT_CARE_DNAND_CHECK_CYCLE));
	}
...
static int oem_get_batt_care_capacity(struct qpnp_qg *chip, int *val)
{
	...
	if (*val > 1) {
		*val = (*val * DEFAULT_BATT_CARE_MODE_CAPACITY) / 100;
	}
...
static int oem_suspend_votable(struct qpnp_qg *chip, bool suspend_enable)
{
	...
	rc = vote(chip->usb_icl_votable, BATT_CARE_VOTER, suspend_enable, 0);
...
{
	struct qpnp_qg *chip = container_of(work, struct qpnp_qg,
					    oem_ui_soc_check_work.work);

	if (chip->charge_status == POWER_SUPPLY_STATUS_CHARGING) {
		chip->oem_ui_soc = chip->oem_ui_soc + 1;
	}
	...
	pr_debug("oem_mode_change_flag:%d, oem_ui_soc_increasing_flag:%d, ui_soc:%d, real_soc:%d, charge_status:%d\n",
				chip->oem_mode_change_flag, chip->oem_ui_soc_increasing_flag, chip->oem_ui_soc, chip->oem_real_soc, chip->charge_status);
...
#define THRESHOLD_CAPASITY				30
#define LAPSED_THRESHOLD_TIME			(2 * 7 * 24 * 60 * 60) //two weeks
static bool oem_batt_care_capacity_check(struct qpnp_qg *chip)
{
	if (chip->oem_real_soc <= THRESHOLD_CAPASITY) {
		chip->oem_capacity_flag = false;
	}

	if (chip->lapsed_time > LAPSED_THRESHOLD_TIME &&
			 chip->oem_capacity_flag) {
		return true;
	}
...
#define TOTAL_CONNECT_THRESHOLD_TIME	(12 * 60 * 60) * 14 //Total connect time for two weeks
static bool oem_batt_care_connect_time_check(struct qpnp_qg *chip)
{
	if (chip->lapsed_time > LAPSED_THRESHOLD_TIME &&
			chip->total_connect_time > TOTAL_CONNECT_THRESHOLD_TIME) {
		return true;
	}
...
static void oem_batt_care_notification_check(struct qpnp_qg *chip)
{
	static bool insertion_flag = false;

	if (is_usb_present(chip)) {
		if (!insertion_flag) {
			insertion_flag = true;
			oem_get_current_time(&chip->insertion_time);
			chip->lapsed_time = chip->insertion_time - chip->start_time;

			chip->oem_notification_flag |= oem_batt_care_capacity_check(chip);
		}
	} else {
		if (insertion_flag) {
			insertion_flag = false;
			oem_get_current_time(&chip->removal_time);
			chip->connect_time = chip->removal_time - chip->insertion_time;
			chip->total_connect_time = chip->total_connect_time + chip->connect_time;
			chip->lapsed_time = chip->removal_time - chip->start_time;

			chip->oem_notification_flag |= oem_batt_care_connect_time_check(chip);
		}
	}

	pr_debug("start_time:%li, insertion_time:%li, removal_time:%li\n",
				chip->start_time, chip->insertion_time, chip->removal_time);
	pr_debug("lapsed_time:%li, connect_time:%li, total_connect_time:%li\n",
				chip->lapsed_time, chip->connect_time, chip->total_connect_time);

	if (chip->oem_notification_flag && !chip->oem_batt_care_notification) {
		chip->oem_batt_care_notification = chip->oem_notification_flag;
		power_supply_changed(chip->qg_psy);
	}

	if (chip->lapsed_time > LAPSED_THRESHOLD_TIME &&
			!chip->oem_notification_flag) {
		pr_info("Unattainment of notification conditions, time measurement start again.");
		oem_get_current_time(&chip->start_time);
		chip->total_connect_time = 0;
	}

	pr_debug("oem_batt_care_notification:%d\n", chip->oem_batt_care_notification);
}
...
static int qpnp_qg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&chip->oem_cycle_dnand_work, oem_cycle_dnand_work);
	INIT_DELAYED_WORK(&chip->oem_online_dnand_work, oem_online_dnand_work);
	INIT_DELAYED_WORK(&chip->oem_online_time_work, oem_online_time_work);
	INIT_DELAYED_WORK(&chip->oem_batt_care_work, oem_batt_care_work);
	INIT_DELAYED_WORK(&chip->oem_ui_soc_check_work, oem_ui_soc_check_work);
	mutex_init(&chip->bus_lock);
	mutex_init(&chip->soc_lock);
	mutex_init(&chip->data_lock);
	mutex_init(&chip->oem_cycle.lock);
	mutex_init(&chip->oem_online.lock);
	mutex_init(&chip->oem_batt_care_lock);
	...
	chip->oem_batt_care_mode = BATT_CARE_MODE_INIT;
	oem_get_current_time(&chip->start_time);
	chip->connect_time = 0;
	chip->lapsed_time = 0;
	chip->total_connect_time = 0;
...
static int qpnp_qg_remove(struct platform_device *pdev)
{
	...
	cancel_delayed_work_sync(&chip->oem_cycle_dnand_work);
	cancel_delayed_work_sync(&chip->oem_online_dnand_work);
	cancel_delayed_work_sync(&chip->oem_online_time_work);
	cancel_delayed_work_sync(&chip->oem_batt_care_work);
	cancel_delayed_work_sync(&chip->oem_ui_soc_check_work);
	device_destroy(chip->qg_class, chip->dev_no);
	cdev_del(&chip->qg_cdev);
	unregister_chrdev_region(chip->dev_no, 1);
	...
	mutex_destroy(&chip->oem_cycle.lock);
	mutex_destroy(&chip->oem_batt_care_lock);
