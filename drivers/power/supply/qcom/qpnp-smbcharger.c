...
#endif
	struct delayed_work		oem_apsd_rerun_work;
	struct delayed_work		oem_floating_cherger_work;
};
...
/* Thermal mitigation voters */
#define OEM_THERMAL_FCC_VOTER	"OEM_THERMAL_FCC_VOTER"
...
/*-------*/
#define OEM_APSD_RERUN_DELAY_MS		3000
...
static int oem_current_comp_disable(struct smbchg_chip *chip);
static int oem_get_battery_info(void);
/*-------*/
...
static void smbchg_usb_update_online_work(struct work_struct *work)
{
	...
	online = chip->usb_present && !chip->very_weak_charger;
								...
								int lvl_sel)
{
	...
	if (chip->therm_lvl_sel == 0) {
		...
		rc = vote(chip->fcc_votable, OEM_THERMAL_FCC_VOTER, false, 0);
		if (rc < 0) {
			pr_err("Can't disable FCC vote rc=%d\n", rc);
		}
#else
...
#ifndef QUALCOMM_ORIGINAL_FEATURE
		rc = vote(chip->fcc_votable, OEM_THERMAL_FCC_VOTER, true,
					thermal_icl_ma);
		if (rc < 0) {
			pr_err("Can't set FCC fcc_ma=%d rc=%d\n", thermal_icl_ma, rc);
		}
...
static int smbchg_config_chg_battery_type(struct smbchg_chip *chip)
{
	...
	union power_supply_propval prop = {0,};
	int battery_info = OEM_BATT_6000MAH;
	...
	battery_info = oem_get_battery_info();
	if (battery_info == OEM_BATT_3000MAH) {
		batt_node = of_parse_phandle(node, "qcom,battery-data-half", 0);
		if (!batt_node) {
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	schedule_delayed_work(&chip->oem_apsd_rerun_work,
					msecs_to_jiffies(OEM_APSD_RERUN_DELAY_MS));
...
static void handle_usb_removal(struct smbchg_chip *chip)
{
	...
	cancel_delayed_work_sync(&chip->oem_apsd_rerun_work);

	pr_debug("clear wakelock\n");
	smbchg_relax(chip, OEM_DETECT_CHGER);
}
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	smbchg_stay_awake(chip, OEM_DETECT_CHGER);
}
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_OEM_LOWBATT_BOOT:
		chip->oem_lowbatt_boot = val->intval;
		break;
#ifdef CONFIG_OEM_HKADC
	case POWER_SUPPLY_PROP_USB_TEMP_LEVEL:
		chip->oem_usb_temp_level = val->intval;
		break;
#endif
	case POWER_SUPPLY_PROP_OEM_INPUT_SUSPEND:
		vote(chip->usb_suspend_votable, OEM_CMD_VOTER, val->intval, 0);
		break;
	case POWER_SUPPLY_PROP_OEM_BATTFET_SUSPEND:
		vote(chip->battchg_suspend_votable, OEM_CMD_VOTER, val->intval, 0);
		break;
	default:
		return -EINVAL;
	}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_OEM_THERMAL_LEVELS:
		...
		break;
	case POWER_SUPPLY_PROP_OEM_LOWBATT_BOOT:
		val->intval = chip->oem_lowbatt_boot;
		break;
#ifdef CONFIG_OEM_HKADC
	case POWER_SUPPLY_PROP_USB_TEMP_LEVEL:
		val->intval = chip->oem_usb_temp_level;
		break;
#endif
	case POWER_SUPPLY_PROP_OEM_INPUT_SUSPEND:
		val->intval = get_client_vote(chip->usb_suspend_votable,
					      OEM_CMD_VOTER);
		break;
	case POWER_SUPPLY_PROP_OEM_BATTFET_SUSPEND:
		val->intval = get_client_vote(chip->battchg_suspend_votable,
					      OEM_CMD_VOTER);
		break;
	default:
		return -EINVAL;
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	/* disable current compensation */
	rc = oem_current_comp_disable(chip);
	if (rc < 0) {
		pr_err("Couldn't disable current compensation rc = %d\n", rc);
	}
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	bool oem_enable_dcin_priority = false;
	...
	oem_enable_dcin_priority = of_property_read_bool(node, "oem,enable-dcin-priority");
	if (oem_enable_dcin_priority) {
		rc = smbchg_sec_masked_write(chip,
				chip->usb_chgpth_base + APSD_CFG,
				INPUT_PRIORITY, 0);
		if (rc < 0)
			pr_err("Couldn't enable DCIN_PRIORITY rc=%d\n", rc);
	}
...
/*-------*/
#define SL_CHG_I_COMP_MASK	SMB_MASK(1, 0)
static int oem_current_comp_disable(struct smbchg_chip *chip)
{
	int rc;
	u8 val = 0;
	...
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + CHGR_CCMP_CFG,
						SL_CHG_I_COMP_MASK, val);

	return rc;
}

static bool oem_distinction_floating_charger(struct smbchg_chip *chip)
{
	int rc;
	int current_limit = 0;
	union power_supply_propval prop = {0,};
	...
	if (usb_supply_type == POWER_SUPPLY_TYPE_USB &&
		...
		floating_charger = true;
	}
	pr_debug("distinction result of floating-charger:%d\n", floating_charger);

	return floating_charger;
}

static void oem_smbchg_apsd_rerun_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				oem_apsd_rerun_work.work);
	int rc;

	if (oem_distinction_floating_charger(chip)) {
		/* re-run APSD */
		pr_debug("APSD RE-RUN\n");
		rc = smbchg_masked_write(chip,
				chip->usb_chgpth_base + USB_CMD_APSD,
				APSD_RERUN, APSD_RERUN);
		schedule_delayed_work(&chip->oem_floating_cherger_work,
			msecs_to_jiffies(OEM_APSD_RERUN_DELAY_MS));
	}
}

static void oem_smbchg_floating_cherger_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				oem_floating_cherger_work.work);

	if (oem_distinction_floating_charger(chip)) {
		pr_debug("determined to be floating-charger\n");
		smbchg_set_sdp_current(chip, CURRENT_500_MA);
	}
}
...
static int smbchg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&chip->oem_apsd_rerun_work, oem_smbchg_apsd_rerun_work);
	INIT_DELAYED_WORK(&chip->oem_floating_cherger_work, oem_smbchg_floating_cherger_work);
