...
static void get_config_work(struct work_struct *work)
{
	...
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++) {
		...
		if (chip->step_chg_config->fcc_cfg[i].value != 0) {
			chip->step_chg_config->number_of_steps++;
		}
	}
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++) {
		...
		if (chip->jeita_fcc_config->fcc_cfg[i].high_threshold != 0) {
			chip->jeita_fcc_config->number_of_steps++;
		}
	}
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++) {
		...
		if (chip->jeita_fv_config->fv_cfg[i].value != 0) {
			chip->jeita_fv_config->number_of_steps++;
		}
	}
...
static int get_val(struct range_data *range, int hysteresis, int current_index,
		int threshold,
		int *new_index, int *val, int process, int steps)
{
	int i;
	int jeita_normal_index = (steps / 2);
	...
	*new_index = -EINVAL;
	...
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++) {
		...
		if (is_between(range[i].low_threshold,
			...
			if (current_index == -EINVAL) {
				pr_debug("First try to find the matching index without hysteresis, new_index:%d", i);
				*new_index = i;
				*val = range[i].value;
			} else {
				pr_debug("Change 1 step at a time according to the value\n");
				if (i > current_index) {
	...
	if (process == STEP_CHG) {
		/*
		 * During STEP_CHG, control the hysteresis only when the voltage drops.
		 */
		...
		/*
		 * During JEITA, control the hysteresis only from low temperature
		 * to normal temperature and from high temperature to normal temperature.
		 */
		if (*new_index == current_index + 1 && current_index < jeita_normal_index) {
		...
		} else if (*new_index == current_index - 1 && current_index > jeita_normal_index) {
			if (threshold > range[*new_index].high_threshold - hysteresis) {
				/*
				 * stay in the current index, threshold is not lower
				 * by hysteresis amount
...
static int handle_jeita(struct step_chg_info *chip)
{
	...
	if (chip->jeita_fcc_index <= 0 ||
			chip->jeita_fcc_index >= (chip->jeita_fcc_config->number_of_steps - 1)) {
		...
		bool step_chg_enable, bool sw_jeita_enable)
{
	...
	chip->step_chg_config->number_of_steps =  0;
	...
	chip->jeita_fcc_config->hysteresis = 20;
	chip->jeita_fcc_config->number_of_steps = 0;
	chip->jeita_fv_config->psy_prop = POWER_SUPPLY_PROP_TEMP;
	chip->jeita_fv_config->prop_name = "BATT_TEMP";
	chip->jeita_fv_config->hysteresis = 20;
	chip->jeita_fv_config->number_of_steps =  0;
