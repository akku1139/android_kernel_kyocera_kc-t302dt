...
/*-------*/
#ifdef CONFIG_OEM_BMS
#define OEM_JEITA_VOTER						"OEM_JEITA_VOTER"
#define PROFILE_VOTER						"PROFILE_VOTER"
...
struct fg_chip {
	...
	/* oem add */
#ifdef CONFIG_OEM_BMS
	int						oem_default_cc_cv_threshold_mv;
	int						oem_default_chg_iterm;
	int						oem_jeita_chg_iterm;
	int						oem_jeita_cccv_comp_mv;
	int						oem_cool_hysteresis;
	int						oem_warm_hysteresis;
	int						oem_battery_info;
	struct votable			*cccv_thresh_votable;
	struct delayed_work		oem_release_of_full_work;
#endif
};
...
/*-------*/
#ifdef CONFIG_OEM_BMS
static int oem_is_factory_setting(void);
static void oem_fg_jeita_soft_lim_config(struct fg_chip *chip);
static void oem_fg_full_soc_configure(struct fg_chip *chip);
static int oem_bms_get_deterioration_status(struct fg_chip *chip);
static void oem_soft_hysteresis_config(struct fg_chip *chip);
static void oem_battery_info_check(struct fg_chip *chip);
#endif
/*-------*/
...
#ifdef CONFIG_OEM_BMS
			cancel_delayed_work_sync(&chip->oem_release_of_full_work);

			if(!chip->charge_full)
				oem_fg_full_soc_configure(chip);
#endif
...
#ifdef CONFIG_OEM_BMS
	if (chip->status != POWER_SUPPLY_STATUS_FULL && chip->charge_full) {
		pr_info("call charge_full_work \n");
...
#ifdef CONFIG_OEM_BMS
	oem_soft_hysteresis_config(chip);
#endif
}
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_OEM_BMS_BATT_STATUS:
		val->intval = oem_bms_get_deterioration_status(chip);
		break;
	case POWER_SUPPLY_PROP_OEM_BMS_FACT_SETTING:
		val->intval = oem_is_factory_setting();
		break;
#endif
	default:
		return -EINVAL;
...
#ifdef CONFIG_OEM_BMS
		oem_fg_jeita_soft_lim_config(chip);
#endif
		break;
...
#ifdef CONFIG_OEM_BMS
	oem_battery_info_check(chip);

	if (chip->oem_battery_info == OEM_BATT_3000MAH) {
		batt_node = of_find_node_by_name(node, "qcom,battery-data-half");
		if (!batt_node) {
...
#ifdef CONFIG_OEM_BMS
	chip->oem_default_cc_cv_threshold_mv = chip->cc_cv_threshold_mv;
#endif
...
#else
	vote(chip->cccv_thresh_votable, PROFILE_VOTER, true, chip->oem_default_cc_cv_threshold_mv);
#endif
...
#ifdef CONFIG_OEM_BMS
#define OEM_DELAYED_RELEASE			2000
#endif
...
#ifdef CONFIG_OEM_BMS
	{
		pr_debug("Release of charge_full flag is delayed.\n");
		schedule_delayed_work(&chip->oem_release_of_full_work,
				msecs_to_jiffies(OEM_DELAYED_RELEASE));
	}
#else
...
#ifdef CONFIG_OEM_BMS
	rc = of_property_read_u32(node, "qcom,fg-chg-iterm-ma",
			&chip->oem_default_chg_iterm);
	if (rc < 0) {
		pr_err("Error reading fg-chg-iterm-ma rc=%d\n",
			rc);
		return rc;
	}

	rc = of_property_read_u32(node, "oem,jeita-chg-iterm-ma",
			&chip->oem_jeita_chg_iterm);
	if (rc < 0) {
		pr_err("Error reading jeita-chg-iterm-ma rc=%d\n",
			rc);
		return rc;
	}

	rc = of_property_read_u32(node, "oem,jeita-cc-cv-comp-mv",
			&chip->oem_jeita_cccv_comp_mv);
	if (rc < 0) {
		pr_err("Error reading jeita-cc-cv-comp-mv rc=%d\n",
			rc);
		return rc;
	}

	rc = of_property_read_u32_array(node,
			"oem,cool-warm-jeita-hysteresis", temp, 2);
	if (rc) {
		pr_err("Error reading cool-warm-jeita-hysteresis rc=%d\n",
			rc);
		return rc;
	}
	chip->oem_cool_hysteresis = temp[0];
	chip->oem_warm_hysteresis = temp[1];

	pr_debug("cool_hysteresis = %d, warm_hysteresis = %d\n",
				chip->oem_cool_hysteresis, chip->oem_warm_hysteresis);
...
static int oem_is_factory_setting(void)
{
	return oem_fact_get_option_bit(OEM_FACT_OPTION_ITEM_01, 6);
}

static void oem_fg_jeita_soft_lim_config(struct fg_chip *chip)
{
	if (chip->health == POWER_SUPPLY_HEALTH_WARM) {
		settings[FG_MEM_TERM_CURRENT].value = chip->oem_default_chg_iterm;
		settings[FG_MEM_CHG_TERM_CURRENT].value = chip->oem_jeita_chg_iterm;
		vote(chip->cccv_thresh_votable, OEM_JEITA_VOTER, true, (chip->batt_max_voltage_uv / 1000) - chip->oem_jeita_cccv_comp_mv);
		pr_info("set param for warm item %d chg_iterm %d cc_cv_th %d \n",
					settings[FG_MEM_TERM_CURRENT].value,
					settings[FG_MEM_CHG_TERM_CURRENT].value,
					(chip->batt_max_voltage_uv / 1000) - chip->oem_jeita_cccv_comp_mv);
	}
	else if (chip->health == POWER_SUPPLY_HEALTH_COOL) {
		settings[FG_MEM_TERM_CURRENT].value = chip->oem_default_chg_iterm;
		settings[FG_MEM_CHG_TERM_CURRENT].value = chip->oem_default_chg_iterm;
		vote(chip->cccv_thresh_votable, OEM_JEITA_VOTER, true, (chip->batt_max_voltage_uv / 1000) - chip->oem_jeita_cccv_comp_mv);
		pr_info("set param for cool item %d chg_iterm %d cc_cv_th %d \n",
					settings[FG_MEM_TERM_CURRENT].value,
					settings[FG_MEM_CHG_TERM_CURRENT].value,
					(chip->batt_max_voltage_uv / 1000) - chip->oem_jeita_cccv_comp_mv);
	}
		...
		settings[FG_MEM_TERM_CURRENT].value = chip->oem_default_chg_iterm;
		settings[FG_MEM_CHG_TERM_CURRENT].value = chip->oem_default_chg_iterm;
		vote(chip->cccv_thresh_votable, OEM_JEITA_VOTER, false, 0);
		pr_info("set param for normal item %d chg_iterm %d cc_cv_th %d \n",
					settings[FG_MEM_TERM_CURRENT].value,
					settings[FG_MEM_CHG_TERM_CURRENT].value,
					chip->oem_default_cc_cv_threshold_mv);
	}
	update_iterm(chip);
	update_chg_iterm(chip);
}
...
static void oem_fg_full_soc_configure(struct fg_chip *chip)
{
	...
	if (fg_debug_mask & FG_STATUS) {
		bsoc = buffer[0] | buffer[1] << 8 | buffer[2] << 16;
		pr_info("wrote %06x into MONOTONIC_SOC\n", bsoc);
	}
...
static int oem_cccv_thresh_set(struct votable *votable, void *data,
			int cccv_thresh, const char *client)
{
	struct fg_chip *chip = data;
	int rc = 0;

	pr_debug("client:%s, set cccv_thresh:%d\n", client, cccv_thresh);

	if (cccv_thresh <= 0) {
		pr_err("Invalid cccv_thresh %d\n", cccv_thresh);
		return -EINVAL;
	}

	chip->cc_cv_threshold_mv = cccv_thresh;
	update_cc_cv_setpoint(chip);

	return rc;
}

static void oem_release_of_full_flag_work(struct work_struct *work)
{
       struct fg_chip *chip = container_of(work,
                               struct fg_chip,
                               oem_release_of_full_work.work);

       pr_debug("FALSE is set to charge_full flag.\n");
       chip->charge_full = false;
}

#define FULL_CC_3000MAH		3000000
#define FULL_CC_6000MAH		6000000
#define DETERIORATION_THRESH_GOOD				75
#define DETERIORATION_THRESH_NORM				55
#define DETERIORATION_THRESH_NORM_TO_GOOD		85
#define DETERIORATION_THRESH_DEAD_TO_NORM		65
...
static int oem_bms_get_deterioration_status(struct fg_chip *chip)
{
	...
	int full_cc_size = FULL_CC_6000MAH;
	...
	if (chip->oem_battery_info == OEM_BATT_3000MAH) {
		full_cc_size = FULL_CC_3000MAH;
	}
	deterioration_degree = (int)((chip->learning_data.learned_cc_uah * 100) / full_cc_size);

	pr_debug("deterioration_degree:%d learned_cc_uah:%lld FULL_CC_UAH:%d\n",
		deterioration_degree, chip->learning_data.learned_cc_uah, full_cc_size);

	switch (deterioration_status) {
	case DETERIORATION_STATUS_INIT:
		if (deterioration_degree >= DETERIORATION_THRESH_GOOD) {
			new_deterioration_status = DETERIORATION_STATUS_GOOD;
		}
		else if (deterioration_degree >= DETERIORATION_THRESH_NORM) {
	...
	case DETERIORATION_STATUS_NORM:
		if (deterioration_degree >= DETERIORATION_THRESH_NORM_TO_GOOD) {
			new_deterioration_status = DETERIORATION_STATUS_GOOD;
		}
		else if (deterioration_degree >= DETERIORATION_THRESH_NORM) {
			new_deterioration_status = DETERIORATION_STATUS_NORM;
	...
	case DETERIORATION_STATUS_DEAD:
		if (deterioration_degree >= DETERIORATION_THRESH_DEAD_TO_NORM) {
			new_deterioration_status = DETERIORATION_STATUS_NORM;
		}
		else {
			new_deterioration_status = DETERIORATION_STATUS_DEAD;
...
static void oem_soft_hysteresis_config(struct fg_chip *chip) {
	...
	switch (chip->health) {
		case POWER_SUPPLY_HEALTH_WARM:
			if (!batt_warm) {
				set_prop_jeita_temp(chip, FG_MEM_SOFT_HOT, warm_temp - chip->oem_warm_hysteresis);
				batt_warm = !batt_warm;
				if (fg_debug_mask & FG_STATUS)
					pr_info("restore soft hot threshold: old hot=%d, new hot=%d\n",
						warm_temp,
						warm_temp - chip->oem_warm_hysteresis);
			}
			break;
		case POWER_SUPPLY_HEALTH_COOL:
			if (!batt_cool) {
				set_prop_jeita_temp(chip, FG_MEM_SOFT_COLD, cool_temp + chip->oem_cool_hysteresis);
				batt_cool = !batt_cool;
				if (fg_debug_mask & FG_STATUS)
					pr_info("restore soft cold threshold: old cold=%d, new cold=%d\n",
						cool_temp,
						cool_temp + chip->oem_cool_hysteresis);
			}
			break;
...
#define DNAND_BATERY_INFO_OFFSET	100
#define BATTERY_DATA_HALF			"UBYD133U"
#define BATTERY_DATA_LEN			8
#define CHG_TERM_HALF_MA			150
#define JEITA_CHG_TERM_HALF_MA		100
static void oem_battery_info_check(struct fg_chip *chip)
{
	...
	if (smem_ptr == NULL) {
		...
		if(strncmp(&smem_ptr[DNAND_BATERY_INFO_OFFSET], BATTERY_DATA_HALF, BATTERY_DATA_LEN) == 0) {
			chip->oem_battery_info = OEM_BATT_3000MAH;
			chip->oem_default_chg_iterm = CHG_TERM_HALF_MA;
			chip->oem_jeita_chg_iterm = JEITA_CHG_TERM_HALF_MA;
			settings[FG_MEM_TERM_CURRENT].value = chip->oem_default_chg_iterm;
			settings[FG_MEM_CHG_TERM_CURRENT].value = chip->oem_jeita_chg_iterm;
		}
	}
...
#ifdef CONFIG_OEM_BMS
	chip->cccv_thresh_votable = create_votable("CCCV_THRESH",
						VOTE_MIN,
						oem_cccv_thresh_set, chip);
	if (IS_ERR(chip->cccv_thresh_votable)) {
		rc = PTR_ERR(chip->cccv_thresh_votable);
		chip->cccv_thresh_votable = NULL;
		goto of_init_fail;
	}
#endif
...
#ifdef CONFIG_OEM_BMS
	INIT_DELAYED_WORK(&chip->oem_release_of_full_work, oem_release_of_full_flag_work);
#endif
