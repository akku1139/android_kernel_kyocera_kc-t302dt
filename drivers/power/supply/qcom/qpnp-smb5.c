...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	if (of_property_read_bool(node, "oem,auto-recharge-soc-disable")) {
		chip->dt.auto_recharge_soc = -EINVAL;
	}
	...
	chg->oem_batt_uvlo_thresh = -EINVAL;
	of_property_read_u32(node, "oem,batt-uvlo-thresh",
					&chg->oem_batt_uvlo_thresh);
...
#ifdef CONFIG_OEM_HKADC
	case POWER_SUPPLY_PROP_USB_TEMP_LEVEL:
		rc = vote(chg->usb_icl_votable, OEM_USB_TEMP_EN_VOTER, (bool)val->intval, 0);
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (chg->oem_batt_uvlo_thresh != -EINVAL) {
		rc = smblib_write(chg, DCDC_BAT_UVLO_THRESHOLD_CFG_REG,
				chg->oem_batt_uvlo_thresh);
		if (rc < 0) {
			pr_err("Couldn't configure BAT_UVLO_THRESHOLD rc=%d\n", rc);
		}
	}
