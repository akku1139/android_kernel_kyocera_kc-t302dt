...
#define AW8695_RTP_FUNC_ENABLE_TIME 100
#define NO_USE 0
...
static int aw8695_haptic_play_go(struct aw8695 *aw8695, bool flag)
{
    ...
    if(((aw8695->state == 0) && ((reg_val&0x0f) == 0x00)) || ((aw8695->state != 0) && ((reg_val&0x0f) == 0x07)))
		return 0;
    if(flag == true) {
...
static void aw8695_rtp_work_routine(struct work_struct *work)
{
    ...
    /* set timer to run timer func*/
    d_val = aw8695->duration;
    hrtimer_start(&aw8695->timer,
                         ktime_set(d_val  / 1000, (d_val  % 1000) * 1000000),
                         HRTIMER_MODE_REL);
...
static int aw8695_haptic_init(struct aw8695 *aw8695)
{
	...
	pr_err("%s wj-->disable f0 calibration\n", __func__);
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
  ...
  //  schedule_work(&aw8695->vibrator_work);
   ...
   if(aw8695->state)
   {
        if(aw8695->duration <= AW8695_RTP_FUNC_ENABLE_TIME)
        {
            //aw8695_haptic_play_mode(aw8695, AW8695_HAPTIC_RAM_MODE);
			//f0_cali_lra
			aw8695_i2c_write(aw8695, AW8695_REG_TRIM_LRA, 0x00);
            aw8695_haptic_set_wav_seq(aw8695,0x01,0x03);
            aw8695_haptic_set_wav_seq(aw8695,0x02,0x00);
            aw8695_haptic_set_wav_loop(aw8695,0x01,0x00);
            aw8695_haptic_brightness_set(&aw8695->cdev,1);
        }else{
            aw8695_haptic_play_mode(aw8695, AW8695_HAPTIC_RTP_MODE);
#if 0
			d_val  = aw8695->duration;
...
static void aw8695_vibrator_work_routine(struct work_struct *work)
{
    ...
    aw8695_haptic_stop(aw8695);
    if(aw8695->state) {
        if(aw8695->activate_mode == AW8695_HAPTIC_ACTIVATE_RAM_MODE && NO_USE) {
            aw8695_haptic_ram_vbat_comp(aw8695, true);
 ...
 *
 *****************************************************/
 static int aw8695_pinctrl_init(struct i2c_client *i2c)
{
	struct pinctrl *ctrl_pdata;
	int rc = -EFAULT;

	ctrl_pdata = kzalloc(sizeof(struct pinctrl),GFP_KERNEL);
	if(!ctrl_pdata)
		return -ENOMEM;
	ctrl_pdata->pinctrl_ctrl = devm_pinctrl_get(&i2c->dev);
		if (IS_ERR_OR_NULL(ctrl_pdata->pinctrl_ctrl)) {
			pr_err("%s: failed to get pinctrl\n", __func__);
			return PTR_ERR(ctrl_pdata->pinctrl_ctrl);
		}

	ctrl_pdata->gpio_state_on= pinctrl_lookup_state(ctrl_pdata->pinctrl_ctrl,"hap_on");
	if (IS_ERR_OR_NULL(ctrl_pdata->gpio_state_on))
		pr_warn("%s: can not get default pinstate\n", __func__);

	ctrl_pdata->gpio_state_off= pinctrl_lookup_state(ctrl_pdata->pinctrl_ctrl,"hap_off");
	if (IS_ERR_OR_NULL(ctrl_pdata->gpio_state_off))
		pr_warn("%s: can not get default pinstate\n", __func__);

	rc = pinctrl_select_state(ctrl_pdata->pinctrl_ctrl, ctrl_pdata->gpio_state_on);
	if (rc)
		dev_err(&i2c->dev,"%s: can not set  pins\n", __func__);
	rc = pinctrl_select_state(ctrl_pdata->pinctrl_ctrl, ctrl_pdata->gpio_state_off);
	if (rc)
		dev_err(&i2c->dev,"%s: can not set  pins\n", __func__);
