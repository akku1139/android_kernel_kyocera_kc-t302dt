		...
		const struct firmware *fw)
{
	...
	if (upgrade > 0)
		...
		if((ld->si->cydata.fw_ver_conf >> 8) != 0x18){
			pr_err("%s ver_conf = %x\n",__func__, (ld->si->cydata.fw_ver_conf>>8));
			return 1;
		}else{
			if(((ts_panel_id&PANEL_ID_BIT_MASK) && (ld->si->cydata.fw_ver_conf&0x80)) ||
				(!(ts_panel_id&PANEL_ID_BIT_MASK) && !(ld->si->cydata.fw_ver_conf&0x80))) {
					pr_err("%s FW type mismatch. ver_conf = %x Panel ID = %x\n",__func__, (ld->si->cydata.fw_ver_conf), ts_panel_id);
					return 1;
			}
		}
	}
...
#define FILENAME_LEN_MAX 64
	...
	if ((ts_panel_id & PANEL_ID_BIT_MASK) == PANEL_ID_TYPE0) {
		snprintf(filename, FILENAME_LEN_MAX, "%s", CY_FW1_FILE_NAME);
	} else {
		snprintf(filename, FILENAME_LEN_MAX, "%s", CY_FW2_FILE_NAME);
	}
...
static int upgrade_ttconfig_from_builtin(struct device *dev)
{
	...
	if ((ts_panel_id & PANEL_ID_BIT_MASK) == PANEL_ID_TYPE0) {
		dev_info(dev, "%s: Filename: %s\n", __func__, CY_CFG1_FILE_NAME);
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY_CFG1_FILE_NAME, dev, GFP_KERNEL, dev,
				_cyttsp5_ttconfig_cont_builtin);
	} else {
		dev_info(dev, "%s: Filename: %s\n", __func__, CY_CFG2_FILE_NAME);
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY_CFG2_FILE_NAME, dev, GFP_KERNEL, dev,
				_cyttsp5_ttconfig_cont_builtin);
	}
