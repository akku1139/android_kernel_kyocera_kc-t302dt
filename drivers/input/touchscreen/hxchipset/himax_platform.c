...
#ifdef TP_IOVCC_1P8V
int tp_vreg_power(int on)
{
	struct himax_i2c_platform_data *pdata= private_ts->pdata;
	int error = 0;
	if(on)
	{
		E("mzy >>>> TP %s on\n", pdata->tp_vreg->vreg_name);
		error = regulator_enable(pdata->tp_vreg->iovcc);
		if(error){
			E("%pS->%s: %s enable vltg fail\n",
	...
	{
		E("mzy >>>> TP %s off\n", pdata->tp_vreg->vreg_name);
		error = regulator_disable(pdata->tp_vreg->iovcc);
		if(error){
			E("%pS->%s: %s disable vltg fail\n",
				__builtin_return_address(0),
				__func__,
				pdata->tp_vreg->vreg_name);
		}
					...
					struct himax_i2c_platform_data *pdata)
{
	...
	if (!gpio_is_valid(pdata->gpio_3v3_en)) {
    	...
    	pdata->gpio_3v3_lcd_tp_shared_3v3_en = of_get_named_gpio(dt, "himax,lcd-tp-shared_vdd1-gpio", 0);

	if (!gpio_is_valid(pdata->gpio_3v3_lcd_tp_shared_3v3_en)) {
		I(" DT:gpio_3v3_en value is not valid\n");
	}
	pdata->gpio_fw = of_get_named_gpio(dt, "himax,fw-gpio", 0);

	if (!gpio_is_valid(pdata->gpio_fw)) {
		I(" DT:gpio_fw is not valid\n");
	}
...
#ifdef TP_IOVCC_1P8V
	#define POWER_OFF_1V8	1

	pdata->tp_vreg = kzalloc(sizeof(struct tp_regulator), GFP_KERNEL);
	if(!pdata->tp_vreg)
	{
		E("%s: request iovcc failed\n", __func__);
		return -ENOMEM;
	}
	/* vreg-name */
	snprintf(pdata->tp_vreg->vreg_name, ARRAY_SIZE("vdd1"), "%s", "vdd1");	//vdd1 regulator had been added in lcd driver

	pdata->power = tp_vreg_power;
	pdata->powerOff3V3 = POWER_OFF_1V8;
#endif
...
void himax_vdd_enable(int pinnum, int enable)
{
    msleep(2);
    gpio_direction_output(pinnum, enable);
    msleep(2);
}
...
#ifdef TP_IOVCC_1P8V
	//#define IOVCC_VREG 1800000
	pdata->tp_vreg->iovcc = regulator_get(&client->dev, "vdd1");
	error = PTR_RET(pdata->tp_vreg->iovcc);
	if (error) {
		E("%pS->%s: %s get failed. error=%d\n",
			 __builtin_return_address(0), __func__,
			pdata->tp_vreg->vreg_name, error);
		pdata->tp_vreg->iovcc = NULL;
		return error;
	}
#endif
