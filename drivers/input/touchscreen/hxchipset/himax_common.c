...
#else
#define ROOTFS_CMDLINE "androidboot.mode=charger"
int FW_switch_flag = 2;
int FW_supplier_flag = 2;
char *i_CTPM_firmware_name = NULL;
char *i_CTPM_firmware_name_LCE_B1_B2 = "CB64_LCE_KY_CID0705_D04_C03_0515-064918.bin";
char *i_CTPM_firmware_name_DJN_B1_B2 = "CB64_DJIN_KY_CID0705_D04_C05_0515-064839.bin";
char *i_CTPM_firmware_name_LCE_B3 = "CB64_LCE_KY_CID0711_D07_C0C_0906-013114.bin";
char *i_CTPM_firmware_name_DJN_B3 = "CB64_DJN_KY_CID0711_D07_C0E_0906-013334.bin";
#endif
...
extern void is_tp_require_power(int is_require);
								...
								size_t len, loff_t *pos)
{
	...
	is_tp_require_power(ts->SMWP_enable);
...
static int i_get_FW(void)
{
	...
	I("%s:gpio 13 = %d", __func__,FW_supplier_flag);
	if(FW_supplier_flag == 1){
		if (socinfo_get_platform_version() < 5242880)
			i_CTPM_firmware_name = i_CTPM_firmware_name_LCE_B1_B2;
		else
			i_CTPM_firmware_name = i_CTPM_firmware_name_LCE_B3;
	}else if(FW_supplier_flag == 0){
		if (socinfo_get_platform_version() < 5242880)
			i_CTPM_firmware_name = i_CTPM_firmware_name_DJN_B1_B2;
		else
			i_CTPM_firmware_name = i_CTPM_firmware_name_DJN_B3;
	}else{
		E("%s,FW fail\n", __func__);
	}
...
static int get_usb_present(void)
{
	...
	if (!psy) {
		...
		if (!psy) {
			E("Could not get usb power_supply\n");
			return -ENODEV;
		}
	}

	power_supply_get_property(psy, POWER_SUPPLY_PROP_PRESENT, &pval);
...
void himax_cable_detect_func(bool force_renew)
{
	...
	connect_status = get_usb_present();/* upmu_is_chr_det(); */
	ts = private_ts;
...
static int TP_get_cmdline_config(struct himax_i2c_platform_data *pdata)
{
	char* rootfsmtd_ptr;
	rootfsmtd_ptr = strstr(saved_command_line, ROOTFS_CMDLINE);
	if (rootfsmtd_ptr) {
		FW_switch_flag = 0;
		I(" %s: no updata in no power!\n", __func__);
		return 1;
	}else{
		FW_switch_flag = 1;
		return 1;
...
#endif
	ts->lcd_tp_shared_3v3_en_gpio = pdata->gpio_3v3_lcd_tp_shared_3v3_en;
	ret = himax_gpio_power_config(pdata);
	if(ret)
		goto err_power_failed;

#if !defined(CONFIG_OF) || defined (TP_IOVCC_1P8V)
	if (pdata->power) {
		ret = pdata->power(1);
...
FW_force_upgrade:
	...
	ret= TP_get_cmdline_config(pdata);
	if (!ret) {
		pr_err("don't get cmdline\n");
	}
	FW_supplier_flag = gpio_get_value(pdata->gpio_fw);
	if(FW_switch_flag){
	...
	ts->pdata->abs_pressure_max        = 112;
	ts->pdata->abs_width_min           = 0;
	ts->pdata->abs_width_max           = 112;
...
void himax_chip_common_deinit(void)
{
	...
	if(ts->pdata->tp_vreg)
	{
		kfree(ts->pdata->tp_vreg);
		ts->pdata->tp_vreg = NULL;
	}
#endif
