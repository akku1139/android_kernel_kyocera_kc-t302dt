...
static int accsns_reg_set_optimum_mode_on(void)
{
    ...
    if(reg_data.available){
        ...
        if( err < 0 ) {
            ACC_LOG_ERR("regulator_set_load fail. err=%d", err);
            return err;
        }
#else
        err = regulator_set_optimum_mode(reg_data.vdd_reg, reg_data.on_load_uA);        if( err < 0 ) {
...
static int accsns_reg_set_optimum_mode_off(void)
{
    ...
    if(reg_data.available){
        ...
        if( err < 0 ) {
            ACC_LOG_ERR("regulator_set_load fail. err=%d\n", err);
        }
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0))
        ret = regulator_set_load(reg_data.vdd_reg, reg_data.on_load_uA);
        if( ret < 0 ) {
            ACC_LOG_ERR("regulator_set_load fail. err=%d", ret);
            return -EIO;
        }
#else
