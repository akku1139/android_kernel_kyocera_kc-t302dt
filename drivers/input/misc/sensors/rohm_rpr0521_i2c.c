...
#define BATT_TEMPERATURE_COEFFI 10
#endif
...
#define ALS_EXP_OF_NV_COEF 1000
...
static u16 nv_photosensor_a0[NUM_OF_COLOR] =
	{0x2EE0, 0x2EE0, 0x2EE0, 0x2EE0, 0x2EE0};
...
static u16 nv_photosensor_a3[NUM_OF_COLOR] =
	{0x276A, 0x276A, 0x276A, 0x276A, 0x276A};
...
static u16 nv_photosensor_b3[NUM_OF_COLOR] =
	{0x0CBC, 0x0CBC, 0x0CBC, 0x0CBC, 0x0CBC};
...
static int ps_get_temperature(PS_ALS_DATA *ps_als, int *temperature)
{
	...
	if (!ps_als->psy) {
		ps_als->psy = power_supply_get_by_name("battery");
	}
	if (ps_als->psy) {
		ret = power_supply_get_property(ps_als->psy, POWER_SUPPLY_PROP_TEMP, &propval);
		*temperature = propval.intval/BATT_TEMPERATURE_COEFFI;
	}
#else
...
static void ps_als_monitor_func(struct work_struct *work)
{
	...
	if ((ps_als->power_data.power_ps || ps_als->power_data.power_als) &&
	    !ps_als_is_device_dead(ps_als) && (!ps_als->ps_is_calibrating)) {
