...
#define ACCSNS_X_AXIS_COEF	1
#define ACCSNS_Y_AXIS_COEF	1
#define ACCSNS_Z_AXIS_COEF	1
...
static int accsns_get_acceleration_data(int *xyz)
{
	...
	xyz[0] *= ACCSNS_X_AXIS_COEF;
	xyz[1] *= ACCSNS_Y_AXIS_COEF;
	xyz[2] *= ACCSNS_Z_AXIS_COEF;
...
static int accsns_activate(int flgatm, int flg, int dtime)
{
    ...
    if (flg == 1) {
        ...
        if( err < 0 ) {
            dev_err(&client_accsns->adapter->dev, "regulator_set_load fail. err=%d\n", err);
            return err;
        }
#else
...
static int accsns_register_init(void)
{
    ...
    usleep_range(1000,1000);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0))
    ret = regulator_set_load(reg_data.vdd_reg, reg_data.off_load_uA);
    if( ret < 0 ) {
        dev_err(&client_accsns->adapter->dev, "regulator_set_load fail. ret=%d\n", ret);
        return ret;
    }
#else
...
static struct of_device_id accsns_of_match[] = {
	{ .compatible = ACCSNS_DRIVER_NAME,},
	{ },
};
MODULE_DEVICE_TABLE(of, accsns_of_match);

static SIMPLE_DEV_PM_OPS(accsns_pm, accsns_suspend, accsns_resume);
...
static struct i2c_driver accsns_driver = {
	...
	.of_match_table = accsns_of_match,
	.pm		= &accsns_pm,
	},
};
