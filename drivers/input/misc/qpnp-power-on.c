...
static struct delayed_work resin_work;
static struct qpnp_pon *resin_pon;
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	if(cfg->pon_type == PON_RESIN) {
		if(gpio_keys_smlpress_check(cfg->key_code, !!key_status))
			return -EPERM;
	}
...
static void resin_work_func(struct work_struct *work)
{
	int rc;
	struct qpnp_pon *pon = resin_pon;
...
static irqreturn_t qpnp_resin_irq(int irq, void *_pon)
{
	...
	resin_pon = pon;
	schedule_delayed_work(&resin_work, 0);
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&resin_work, resin_work_func);
