	...
	enum led_brightness value)
{
	...
	if (data->mdm_led &&
		...
		if (request.mode != TRICOLOR_BLINK_REQUEST ||
			...
			__pm_stay_awake(&data->wakeup_source);
	...
	struct tricolor_led_data *data, struct device *parent)
{
	...
	wakeup_source_init(&data->wakeup_source, TRICOLOR_DRV_NAME);
...
fail_led_classdev_register:
	wakeup_source_trash(&data->wakeup_source);
