...
/* Voltage to temperature */
static const struct qpnp_vadc_map_pt adcmap_batt_therm_pu30_b3380[] = {
	{1663,	-400},
	{1639,	-380},
	{1613,	-360},
	{1585,	-340},
...
int32_t qpnp_adc_batt_therm_B3380_pu30 (struct qpnp_vadc_chip *chip,
                                int32_t adc_code,
                                const struct qpnp_adc_properties *adc_properties,
                                const struct qpnp_vadc_chan_properties *chan_properties,
                                struct qpnp_vadc_result *adc_chan_result)
{
        int64_t batt_thm_voltage = 0;
        if (!chan_properties || !chan_properties->offset_gain_numerator
                        ||!chan_properties->offset_gain_denominator || !adc_properties || !adc_chan_result)
         return -EINVAL;
        /* (code * vref_vadc (1.875V) * 1000) / (scale_code * 1000) */
        if (adc_code > QPNP_VADC_HC_MAX_CODE)
        adc_code = 0;
        ...
        batt_thm_voltage = div64_s64(batt_thm_voltage,
        adc_properties->full_scale_code * 1000);
        qpnp_adc_map_voltage_temp(adcmap_batt_therm_pu30_b3380,
        ARRAY_SIZE(adcmap_batt_therm_pu30_b3380),
        batt_thm_voltage, &adc_chan_result->physical);
        return 0;

}
EXPORT_SYMBOL(qpnp_adc_batt_therm_B3380_pu30);
