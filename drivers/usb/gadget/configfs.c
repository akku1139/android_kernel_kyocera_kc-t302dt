						...
						const char *page, size_t len)
{
	...
	if (!gs->serialnumber || strcmp("KYOCERA", gs->serialnumber)) {
	...
	if (android_connect_flag == 1) {
		...
		list_for_each_entry(c, &gi->cdev.configs, list) {
			...
			if (ms_mode_flg) {
				...
				struct gadget_strings *gs;
				list_for_each_entry(gs, &gi->string_list, list) {
					usb_string_copy("KYOCERA", &gs->serialnumber);
				}
			}
	...
	if (!list_empty(&gi->string_list)) {
		...
		list_for_each_entry(gs, &gi->string_list, list) {
			...
			if (!gs->manufacturer || !strcmp("", gs->manufacturer)) {
				usb_string_copy("KYOCERA", &gs->manufacturer);
			}
			gs->strings[USB_GADGET_MANUFACTURER_IDX].s =
				gs->manufacturer;
			if (!gs->product || !strcmp("", gs->product)) {
				usb_string_copy("KYOCERA_Android", &gs->product);
			}
