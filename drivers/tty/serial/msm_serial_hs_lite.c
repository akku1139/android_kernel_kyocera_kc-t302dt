...
exit_uart_config:
	...
	if (!IS_ERR_OR_NULL(msm_hsl_port->pinctrl)) {
		pr_err("Using Pinctrl set_state active\n");
...
#endif
	if (!IS_ERR_OR_NULL(msm_hsl_port->pinctrl)) {
		pr_err("Using Pinctrl set_state inactive\n");
		ret = pinctrl_select_state(msm_hsl_port->pinctrl,
...
static int bus_vote(uint32_t client, int vector)
{
	...
	pr_err("Voting for bus scaling:%d\n", vector);
...
static void msm_hsl_get_pinctrl_configs(struct uart_port *uport)
{
	...
	if (IS_ERR_OR_NULL(msm_hsl_port->pinctrl)) {
		...
		pr_err("Pinctrl state inactive %p\n", set_state);
...
static int msm_hsl_startup(struct uart_port *port)
{
	...
	dev_err(&pdev->dev, "msm_hsl_startup\n");
				...
				const char *buf, size_t count)
{
	...
	switch (enable) {
	case 0:
		pr_err("Calling stop_console\n");
		console_stop(port->cons);
		pr_err("Calling unregister_console\n");
		...
		break;
	case 1:
		pr_err("Calling register_console\n");
		/*
		 * Disable UART Core clk
...
static int msm_serial_hsl_suspend(struct device *dev)
{
	...
	if (port) {
		...
		if (!IS_ERR_OR_NULL(msm_hsl_port->pinctrl)) {
			pr_err("Using Pinctrl set_state suspend\n");
			ret = pinctrl_select_state(msm_hsl_port->pinctrl,
...
static int msm_serial_hsl_resume(struct device *dev)
{
	...
	dev_err(&pdev->dev, "msm_serial_hsl_resume\n");
	if (port) {
		msm_hsl_port = UARTDM_TO_MSM(port);
		if (!IS_ERR_OR_NULL(msm_hsl_port->pinctrl)) {
			pr_err("Using Pinctrl set_state inactive\n");
...
static int msm_hsl_runtime_suspend(struct device *dev)
{
	...
	dev_err(dev, "pm_runtime: suspending\n");
	msm_hsl_deinit_clock(port);
	return 0;
}
...
static int msm_hsl_runtime_resume(struct device *dev)
{
	...
	dev_err(dev, "pm_runtime: resuming\n");
	msm_hsl_init_clock(port);
	return 0;
}
