				    ...
				    unsigned int mctrl)
{
	...
	if (msm_uport->irda)
		msm_hs_write(uport, UART_DM_IRDA, msm_uport->irda_reg);
}
	...
	*msm_hs_dt_to_pdata(struct platform_device *pdev)
{
	...
	pdata->irda = of_property_read_bool(node, "irda");

	if (pdata->irda) {
		ret = of_property_read_u32(node, "irda_reg",
				&irda_reg);
		if (ret < 0) {
			pr_err("Error: irda_reg not specified.\n");
			pdata->irda = false;
			return ERR_PTR(ret);
		}
		pdata->irda_reg = irda_reg;
	}
...
static int msm_hs_probe(struct platform_device *pdev)
{
	...
	msm_uport->irda = pdata->irda;
	msm_uport->irda_reg = pdata->irda_reg;
