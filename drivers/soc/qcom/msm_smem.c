...
typedef struct {
	...
	uint64_t boot_cmdline[ ( KCC_SMEM_BOOT_CMDLINE_SIZE + (sizeof( uint64_t )-1) ) / sizeof( uint64_t ) ];
...
void *kc_smem_alloc(unsigned smem_type, unsigned buf_size)
{
	...
	switch (smem_type)
	{
		...
		case SMEM_BOOT_CMDLINE:
			if( buf_size <= KCC_SMEM_BOOT_CMDLINE_SIZE )
			{
				return( ( void * )&kc_smem_addr->boot_cmdline[ 0 ] );
			}
			break;
