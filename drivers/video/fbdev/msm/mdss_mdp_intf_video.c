		...
		struct mdss_panel_info *pinfo, struct mdss_mdp_video_ctx *ctx)
{
	...
	mutex_lock(&ctl->offlock);
	if (ctx->timegen_en) {
		pr_info("checkpoint: %s: Intf: %d execute OFF sequence START\n", __func__, ctl->intf_num);
		...
		pr_info("checkpoint: %s:  Intf: %d execute OFF sequence END\n", __func__, ctl->intf_num);
	}
...
static int mdss_mdp_video_display(struct mdss_mdp_ctl *ctl, void *arg)
{
	...
	if (!ctx->timegen_en) {
		pr_info("checkpoint: %s: Intf: %d execute ON sequence START\n", __func__, ctl->intf_num);
		...
		mdss_mdp_ctl_intf_event(ctl, MDSS_EVENT_POST_PANEL_ON, NULL,
			CTL_INTF_EVENT_FLAG_DEFAULT);
		pr_info("checkpoint: %s: Intf: %d execute ON sequence END\n", __func__, ctl->intf_num);
	}
