...
static int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	kdisp_com_panel_power_onoff(&ctrl_pdata->kc_ctrl_data, 0);

	if (mdss_dsi_pinctrl_set_state(ctrl_pdata, false))
		pr_debug("reset disable: pinctrl not enabled\n");

	if(pf_sstype == PF_SSTYPE_HINOKI01){
		usleep_range(1000, 2000); /* h/w recommended delay */
	}
...
extern void is_need_pull_down(int is_need);
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	if (!pdata->panel_info.cont_splash_enabled) {
		kdisp_com_panel_power_onoff(&ctrl_pdata->kc_ctrl_data, 1);
	}
...
#ifdef KC_DISP_CUSTOM
	dsi_pan_node = kdisp_connect_find_panel_of_node(pdev);
#else
...
static int mdss_dsi_ctrl_probe(struct platform_device *pdev)
{
	...
	kdisp_com_init(pdev, dsi_pan_node, &ctrl_pdata->kc_ctrl_data);

#ifdef FB_SLR_MODE
	ctrl_pdata->slr_ctrl_data = devm_kzalloc(&pdev->dev, sizeof(struct slr_ctrl_info), GFP_KERNEL);
	if(!ctrl_pdata->slr_ctrl_data)
	{
		pr_info("%s: get slr_ctrl_data memory is failed\n", __func__);
		goto end;
	}
	if(fb_slr_init(dsi_pan_node, ctrl_pdata->slr_ctrl_data))
	{
		pr_info("%s: slr init failed\n", __func__);
		devm_kfree(&pdev->dev, ctrl_pdata->slr_ctrl_data);
		ctrl_pdata->slr_ctrl_data = NULL;
	}
	...
	struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->tp_rst_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
			"qcom,touchscreen-reset-gpio",0);
	if (!gpio_is_valid(ctrl_pdata->tp_rst_gpio))
		pr_err("%s:%d, touchscreen reset gpio not specified\n",
			__func__, __LINE__);
