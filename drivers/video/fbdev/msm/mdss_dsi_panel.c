...
static bool tp_require_power = false;
static bool  need_pull_down = false;

void is_tp_require_power(int is_require)
{
	if(is_require)
		tp_require_power = true;
	else
		tp_require_power = false;
}
EXPORT_SYMBOL_GPL(is_tp_require_power);

void is_need_pull_down(int is_need)
{
	if(is_need)
		need_pull_down = true;
	else
		need_pull_down = false;
}
EXPORT_SYMBOL_GPL(is_need_pull_down);

bool get_tp_power_status(void)
{
	if(need_pull_down)
		return false;
	else
		return tp_require_power;
}
EXPORT_SYMBOL_GPL(get_tp_power_status);
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if(pf_sstype == PF_SSTYPE_HINOKI01){
		if (!gpio_is_valid(ctrl_pdata->tp_rst_gpio)) {
			pr_debug("%s:%d, touchscreen reset line not configured\n",
				__func__, __LINE__);
			return rc;
		}
	}
	...
	if (enable) {
		...
		if(pf_sstype == PF_SSTYPE_HINOKI01) {
			// there is no need to pull down the lcd_reset and tp_reset if tp requires power
			if (!get_tp_power_status())
			{
				gpio_set_value((ctrl_pdata->rst_gpio), 0);
				gpio_set_value((ctrl_pdata->tp_rst_gpio),0);
				pr_err("%s: enable = %d panel reset gpio level= [%d]\n", __func__, enable, gpio_get_value(ctrl_pdata->rst_gpio));
			}
			gpio_free(ctrl_pdata->rst_gpio);
			gpio_free(ctrl_pdata->tp_rst_gpio);
		...
		{
			usleep_range(ctrl_pdata->kc_ctrl_data.lp00_delay, ctrl_pdata->kc_ctrl_data.lp00_delay);
			gpio_set_value((ctrl_pdata->rst_gpio), 0);
			gpio_free(ctrl_pdata->rst_gpio);
			usleep_range(ctrl_pdata->kc_ctrl_data.dsi_off_rst_delay, ctrl_pdata->kc_ctrl_data.dsi_off_rst_delay);
			if (gpio_is_valid(ctrl_pdata->mode_gpio))
				gpio_free(ctrl_pdata->mode_gpio);
		}
	}
...
#ifdef FB_SLR_MODE
	if(ctrl->slr_ctrl_data->current_slr_mode)
		fb_slr_set(ctrl, ctrl->slr_ctrl_data->current_slr_mode);
#endif
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if ((socinfo_get_platform_version() < 5242880) && (socinfo_get_platform_subtype() == PF_SSTYPE_HINOKI01))
		data = 0;
	if (data) {
