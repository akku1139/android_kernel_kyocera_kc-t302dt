...
static bool ibb_init = false;
static bool lab_init = false;
static bool vdd1_init = false;

extern bool get_tp_power_status(void);
	...
	if (enable) {
		for (i = 0; i < num_vreg; i++) {
			...
			if(pf_sstype == PF_SSTYPE_HINOKI01){
				if(ibb_init && (!strcmp(in_vreg[i].vreg_name, "ibb")))
					continue;
				if(lab_init && (!strcmp(in_vreg[i].vreg_name, "lab")))
					continue;
				if(vdd1_init && (!strcmp(in_vreg[i].vreg_name, "vdd1")))
					continue;
			}
			...
			if(pf_sstype == PF_SSTYPE_HINOKI01){
				...
				if(!strcmp(in_vreg[i].vreg_name, "lab"))
					lab_init = true;
				if(!strcmp(in_vreg[i].vreg_name, "vdd1"))
					vdd1_init = true;
			}
		}
	} else {
		for (i = num_vreg-1; i >= 0; i--) {
			if(pf_sstype == PF_SSTYPE_HINOKI01){
				...
				{
					// if tp require power, keep vsp/vsn on when suspend to support tp easy wake.
					if ((!strcmp(in_vreg[i].vreg_name, "ibb")
								|| !strcmp(in_vreg[i].vreg_name, "lab")
								|| !strcmp(in_vreg[i].vreg_name, "vdd1")))
						continue;
				}
			}
