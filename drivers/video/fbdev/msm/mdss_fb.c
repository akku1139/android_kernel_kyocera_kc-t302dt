...
#ifdef FB_SLR_MODE
static DEVICE_ATTR(slr_mode, 0664, fb_slr_get_mode, fb_slr_set_mode);
#endif
			 ...
			 unsigned long arg, struct file *file)
{
	...
	case MSMFB_ATOMIC_COMMIT:
		...
		pr_debug("%s: Start MSMFB_ATOMIC_COMMIT\n", __func__);
		ret = mdss_fb_atomic_commit_ioctl(info, argp, file);
		pr_debug("%s: End MSMFB_ATOMIC_COMMIT\n", __func__);
		break;
