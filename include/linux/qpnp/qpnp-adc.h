...
enum qpnp_adc_scale_fn_type {
	...
	SCALE_THERM_OEM_NONPULL = 15,
#endif /* CONFIG_OEM_HKADC */
 ...
 * @chip:		Structure device for qpnp vadc
...
int32_t qpnp_adc_scale_qrd_batt_therm(struct qpnp_vadc_chip *dev,
			...
			struct qpnp_vadc_result *chan_rslt);
/**
* qpnp_adc_batt_therm_B3380_pu30() - Scales the pre-calibrated digital
output
* of an ADC to the ADC reference and compensates for the
* gain and offset. Returns the temperature in decidegC.
* It uses a mapping table computed for a 400K pull-up.
...
* @chan_rslt: physical result to be stored.
*/
int32_t qpnp_adc_batt_therm_B3380_pu30(struct qpnp_vadc_chip *dev,
			int32_t adc_code,
			const struct qpnp_adc_properties *adc_prop,
			const struct qpnp_vadc_chan_properties*chan_prop,
			struct qpnp_vadc_result *chan_rslt);
...
{ return -ENXIO; }
#ifdef CONFIG_OEM_HKADC
static inline int32_t oem_adc_tdkntcg_therm(struct qpnp_vadc_chip *vadc,
			int32_t adc_code,
			const struct qpnp_adc_properties *adc_prop,
			const struct qpnp_vadc_chan_properties *chan_prop,
			struct qpnp_vadc_result *chan_rslt)
